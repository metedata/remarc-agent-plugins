...
One public semantic version covers the `remarc`, `remarc-hooks`, and
`remarc-wake` distribution metadata:
...
- `.omp-plugin/marketplace.json` entry `remarc-wake`
- `plugins/remarc-wake/package.json`
...
4. Build and test all runtime packages:
...
7. Exercise clean, isolated installs for every supported agent. Record the exact agent versions used. For OMP, use the pinned `omp/17.3.4` binary and the candidate checkout:
   ...
   This run must prove that OMP copied `plugin.json`, `mcp.json`, the skill, MCP
   bundle, and wake extension into its isolated cache rather than executing
   them through a symlink to the checkout. The OMP TUI must list
   `remarc:remarc` as connected under `Agent Plugins`; the installed server
