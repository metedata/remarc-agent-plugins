...
| `type.comment.text` | app | Complete selected text for a text-selection comment. List views may show a bounded preview, while full-detail delivery must preserve the complete value separately from `commentText`. Queue delivery includes the complete value when it fits the hook output limit; an oversized value is truncated before sentinel wrapping so the delimiters stay balanced, followed by a trusted `remarc_get_comment` instruction containing the full UUID. The user's surrounding request supplies the action for a reference-only batch; absent both an instruction body and a surrounding request, an agent must ask rather than invent an action. |
...
| `origin` (Session) | MCP | Which harness created the session. Known values are `manual`, `claudeCode`, `codex`, and `omp`. Claude Code/Codex use their manifest or supported tool override; an OMP-owned server forces `omp` from its trusted launch identity. Runtime readers preserve unknown strings so an older app can round-trip a newer harness, and the JSON Schema plus cross-language fixture cover all four current values. |
...
`remarc_set_status` accepts an optional `expected_status`. The write happens inside the document transaction, so the check and mutation cannot be split by another writer. The wake payload instructs agents to claim with `expected_status: "handedOff"`; at most one cooperative caller can successfully perform that transition from the expected state. Current Claude wake delivery is best-effort: it records the generation before the hook process emits its payload, so a crash at that boundary can lose a notification. OMP writes `pendingWake` before enqueue, retains it because OMP returns no delivery receipt, and advances `wakedAt` only after Remarc durably shows that the comment left `handedOff`; replay after resume may duplicate an attempt. A caller can also crash after claiming. The compare-and-set therefore prevents two cooperative callers from winning the same status transition; it does not make delivery or agent execution exactly-once.

## Contract versioning

The current schema has **no version field**. Both Swift (`AppState`) and TypeScript (`RawAppState`) decoders permit unknown fields for forward-compatibility, and the plugin's JSON-schema validation in CI only fails on missing-required or type-mismatch (additive optional fields pass). Session origins are explicitly enumerated in `comments-schema.json`; adding a harness requires widening that enum and the cross-language fixture while runtime decoders continue to preserve still-unknown strings.
